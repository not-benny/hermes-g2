...
test("tool and text events project bounded summaries without arguments, results, reasoning, prompts, or secrets", () => {
  ...
  const delta = adapter.ingest({ type: "message.delta", session_id: "hermes-private-session", data: { text: "Working safely" } });
  assert.equal(delta.row.text, "Working safely");
  assert.equal(adapter.ingest({ type: "reasoning.delta", session_id: "hermes-private-session", data: { text: "hidden" } }), null);
  assert.equal(adapter.ingest({ type: "message.delta", session_id: "unshared", data: { text: "hidden" } }), null);
});
...
test("clarify requests use opaque handles and stale answer races cannot reach Hermes RPC", () => {
  ...
  assert.equal(adapter.ingest({ type: "clarify.request", session_id: "hermes-private-session", data: {
    request_id: "provider-request-private", question: "Which test target?", options: ["Unit", "Full"],
  } }), null, "duplicate provider request is deduplicated before projection");
  ...
  assert.deepEqual(adapter.handleCommand(command), {
  ...
  assert.equal(adapter.handleCommand(command), null, "one-shot replay is inert");
  ...
  assert.equal(adapter.handleCommand({ ...command, command_id: "command_late_12345" }), null);
});
...
test("permissions are typed exact-scope or deny-only when scope cannot be represented", () => {
  ...
  assert.deepEqual(adapter.handleCommand(allow), {
...
test("steer and interrupt revalidate current shared generation immediately before exact RPC", () => {
  ...
  }), { jsonrpc: "2.0", id: "command_steer_1234", method: "session.steer",
  ...
  }), { jsonrpc: "2.0", id: "command_stop_12345", method: "session.interrupt",
...
test("local fake adapter exercises snapshot, event projection, and exact action RPC over JSON lines", () => {
  const input = [
    ...
    { op: "snapshot" },
    { op: "event", event: { type: "tool.start", session_id: "private-session", data: { name: "read_file", args: { secret: "privacy-sentinel" } } } },
