...
test("window management is a unified menu entry, not a direct grab", () => {
  ...
  assert.match(
    shell,
    /if \(this\.reorderingWindowId !== null\) \{\s*return \{ shell: true, window: false \};\s*\}/,
  );
});

test("while moving in Window Management, scroll moves the tab and a tap drops it", () => {
  const shell = read("app/ui/shell/shell.ts");
  const management = shell.slice(
    shell.indexOf("private handleSidebarInput"),
    shell.indexOf("private moveSelection"),
  );
  ...
  assert.match(
    management,
    /case "double-click":\s*this\.exitWindowManagement\(\)/,
  );
});
...
test("the pinned launcher cannot be reordered and stays first", () => {
  ...
  assert.match(
    shell,
    /private isReorderable\(window: ShellWindow\): boolean \{\s*return window\.closeable !== false;/,
  );
  assert.match(
    shell,
...
test("reordering persists the new order through onWindowsChanged", () => {
  ...
  const moveReorder = shell.slice(
    shell.indexOf("private moveReorder"),
    shell.indexOf("private endReorder"),
  );
...
test("management marker remains visible over the focused white card", () => {
  ...
  assert.match(
    chrome,
    /fillRoundedRect\(cx - 12, cy - 12, 24, 24, SHELL_OPAQUE_BLACK, 4\)/,
  );
...
test("all sidebar tabs scroll within the single optically visible column", () => {
  ...
  assert.match(
    chrome,
    /visibleCount = rowsPerColumn \* SIDEBAR_VISIBLE_COLUMNS/,
  );
  assert.match(
    chrome,
    /column: FIRST_COLUMN,\s*y: listTop \+ position \* itemStride/,
  );
  assert.doesNotMatch(
    chrome,
    /columnsFromRight|drawSelectionBox/,
    "no tab or selection can enter x=0..35",
  );
  assert.match(
    chrome,
    /sidebarLeftColumnUsed\(_windowCount: number\): boolean \{\s*return false;/,
  );
  assert.match(
    chrome,
...
test("Window Management selection is bounded and preserves the closed card slot", async () => {
  const source = ts.transpileModule(
    read("app/ui/shell/window-management-selection.ts"),
    {
      compilerOptions: {
  ...
  const { nextWindowManagementIndex, selectionIndexAfterManagementRemoval } =
...
test("Window Management remains available for a visible Health-only card and advertises only valid moves", () => {
  ...
  assert.match(
    shell,
    /window\.closeable !== false \|\| window === this\.healthWindow/,
  );
