...
const notifications = readFileSync(
  new URL("../app/ui/notifications.ts", import.meta.url),
  "utf8",
);
const pagination = readFileSync(
  new URL("../app/ui/notification-pagination.ts", import.meta.url),
  "utf8",
);
const digestStart = notifications.indexOf(
  "export class NotificationDigestLayer",
);
...
const harness = `
  ...
  const GLASS_RADIUS = { selection: 6, control: 8 };
  const GLASS_TONE = {
    track: 48,
    divider: 64,
    hint: 112,
  ...
  const drawGlassPanel = (image, x, y, width, height) => {
    image.fillRoundedRect(x, y, width, height);
    image.drawRoundedRect(x, y, width, height);
  };
  const drawGlassSelection = (image, x, y, width, height) => {
    image.fillRoundedRect(x, y, width, height);
    image.drawRoundedRect(x, y, width, height);
  };
  class GrayImage {
    ...
    drawLine(x1, y1, x2, y2, value) { this.commands.push({ type: "line", x1, y1, x2, y2, value }); }
...
test("large-font 196px digest keeps every selected entry and its reason visible", () => {
  ...
  for (let selected = 0; selected < entries.length; selected++) {
    ...
    const selectedLine = image.commands.find(
      (command) =>
        command.type === "text" &&
        command.text === `App ${selected}: Title ${selected}`,
    );
    const reasonLine = image.commands.find(
      (command) => command.type === "text" && command.text.startsWith("Why: "),
    );
    const selection = image.commands.find((command) => command.type === "fill");
    assert.equal(selectedLine?.text, `App ${selected}: Title ${selected}`);
    assert.equal(reasonLine?.text, `Why: priority ${selected}`);
    assert.ok(
      selection &&
        selection.y <= selectedLine.y &&
        selection.y + selection.height > reasonLine.y,
      "the shared focus surface must cover the selected title and reason",
    );
    assert.ok(
      selectedLine.y >= 42 && selectedLine.y < 162,
      JSON.stringify(selectedLine),
    );
    assert.ok(
      reasonLine.y >= 42 && reasonLine.y < 162,
      JSON.stringify(reasonLine),
    );
    const footer = image.commands.find(
      (command) => command.text === "Click review · ·· close",
    );
    assert.ok(
      footer.y + 24 <= 196,
      "the digest close hint remains inside the modal",
    );
...
test("large-font 196px detail menu pages uncapped actions and reaches Dismiss", () => {
  ...
  for (let selected = 0; selected < menu.length; selected++) {
    ...
    image.commands = [];
    image.fillRoundedRect = (x, y, width, height) =>
      image.commands.push({ type: "fill", x, y, width, height });
    image.drawRoundedRect = (x, y, width, height) =>
      image.commands.push({ type: "border", x, y, width, height });
    image.drawText = (_font, x, y, text) =>
      image.commands.push({ type: "text", x, y, text });
    ...
    assert.ok(
      highlight.y >= 0 && highlight.y + highlight.height <= 196,
    ...
    assert.ok(
      image.commands.some(
        (command) =>
          command.type === "text" &&
          command.text.includes(menu[selected].label),
      ),
      `selected row ${selected} must be painted`,
    );
  }
  const dismissLayout = runtime.notificationDetailMenuLayout(
    196,
    24,
    menu.length,
    menu.length - 1,
  ...
  assert.equal(
    dismissLayout.end,
    menu.length,
    "Dismiss is the final reachable row",
  );
  assert.ok(
    dismissLayout.capacity >= 1 && dismissLayout.capacity < menu.length,
  );
});
