...
const scriptPath = new URL("../scripts/build-sign-install-from-github.sh", import.meta.url);
...
function assertBefore(earlier, later) {
  ...
  assert.notEqual(earlierIndex, -1, `missing expected script fragment: ${earlier}`);
...
test("installer builds the exact fetched GitHub snapshot through the unsigned release gate", () => {
  ...
  assert.match(script, /DEFAULT_REF="fix\/mcp-glasses-reliability"/);
  ...
  assert.match(script, /bash scripts\/verify-release-artifacts\.sh "\$unsigned_apk"/);
...
test("installer signs without command-line passwords and verifies identity", () => {
  ...
  assert.doesNotMatch(script, /--ks-pass\s+(?:pass:|"?\$HERMES_KEYSTORE_PASSWORD)/);
  ...
  assert.match(script, /APK output path must not overwrite the signing keystore/);
...
test("installer protects existing data, signer, and versionCode", () => {
  ...
  assert.match(script, /could not determine or read the existing app; refusing an unchecked update/);
  ...
  assert.doesNotMatch(script, /"\$\{adb_device\[@\]\}"\s+(?:uninstall|shell pm clear)/);
