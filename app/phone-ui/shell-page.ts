...
export function loaded(args: EventData): void {
  ...
  const tabLayout = (tabs as any)?.nativeViewProtected?.tabLayout as
    ...
    | undefined;
  // NativeScript's Android TabView grid normalizes the native TabLayout's
  // measured minimum by density once more. Compensate at this boundary so the
  // resulting tappable row is the requested 56 DIP rather than about 28 DIP.
