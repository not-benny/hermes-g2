...
export class ConfigViewModel extends Observable {
  ...
  private saveAddresses(): boolean {
    ...
    try {
      ...
      this.status = `Device addresses could not be fully verified. The ring address may already have changed; retry Save before reconnecting: ${(error as Error)?.message ?? String(error)}`;
      return false;
    }
