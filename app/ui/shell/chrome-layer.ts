...
import {
  fitHudRowItemWidths,
  hudNotificationIconPosition,
  layoutHudNotifications,
} from "./hud-layout";
...
const ROWS_PER_COLUMN = Math.max(
  1,
  ((MIN_WINDOW_HEIGHT - TOP_BAR_HEIGHT - 2 * LIST_MARGIN + ICON_SPACING) /
    ICON_STRIDE) |
    0,
);
...
const BORDER_VALUE = GLASS_TONE.divider;
...
export function makeLetterWindowIcon(
  letter: string,
  ...
  return (image, x, y, size, inverted) => {
    ...
    image.drawText(
      ...
      textY,
      letter,
      inverted ? SHELL_OPAQUE_BLACK : 210,
    );
  };
}

/** Window icon rendered from an SVG (Lucide), rendered once per size and cached. */
export function makeSvgWindowIcon(
  name: IconName,
  glyph?: string,
): ShellChromeWindow["drawIcon"] {
  return (image, x, y, size, inverted) => {
    const icon = glyph
      ? renderIconWithGlyph(name, glyph, size)
      ...
      for (let col = 0; col < icon.width; col++) {
        ...
        if (coverage > 0) {
          image.setPixel(
            dx + col,
            dy + row,
            Math.max(SHELL_OPAQUE_BLACK, 255 - coverage),
          );
        }
      }
    }
...
export class ShellChromeLayer implements Layer {
  ...
  private drawSidebar(image: GrayImage, state: ShellChromeState): void {
    ...
    const listTop =
      bandTop + TOP_BAR_HEIGHT + LIST_MARGIN + state.sidebarBounceY;
    ...
    const selVisible =
      state.selectedIndex >= this.scrollRow &&
      state.selectedIndex < lastVisible;
    ...
    for (let index = this.scrollRow; index < lastVisible; index++) {
      ...
      if (window.attention) {
        ...
        image.fillRoundedRect(
          ...
          8,
          8,
          focused ? SHELL_OPAQUE_BLACK : 255,
          4,
        );
      }
    }
    ...
    if (this.scrollRow > 0) {
      drawChevron(
        image,
        SIDEBAR_WIDTH - SIDEBAR_COLUMN_WIDTH / 2,
        bandTop + TOP_BAR_HEIGHT + 6,
        -1,
      );
    }
    ...
    if (state.closing && selSlot) {
      const cx =
        selSlot.column * SIDEBAR_COLUMN_WIDTH +
        ICON_MARGIN_X +
        (ICON_SIZE >> 1);
      ...
      if (state.reordering) {
        ...
        drawChevron(image, cx, cy - 4, -1, GLASS_TONE.focus);
        drawChevron(image, cx, cy + 4, 1, GLASS_TONE.focus);
        drawChevron(image, cx, selSlot.y - 2, -1, GLASS_TONE.focus);
        drawChevron(image, cx, selSlot.y + ICON_SIZE + 3, 1, GLASS_TONE.focus);
        ...
        drawChevron(image, cx, cy + 3, 1, GLASS_TONE.focus);
      } else {
        image.drawRect(cx - 6, cy - 4, 12, 10, GLASS_TONE.focus);
        image.drawLine(cx - 3, cy - 4, cx - 3, cy - 8, GLASS_TONE.focus);
        image.drawLine(cx + 3, cy - 4, cx + 3, cy - 8, GLASS_TONE.focus);
      }
    }
  }
  ...
  private drawTopBar(image: GrayImage, state: ShellChromeState): void {
    ...
    image.fillRect(
      SIDEBAR_WIDTH,
      barTop,
      G2_LENS_WIDTH - SIDEBAR_WIDTH,
      TOP_BAR_HEIGHT,
      SHELL_OPAQUE_BLACK,
    );
    image.drawLine(
      SIDEBAR_WIDTH,
      barTop + TOP_BAR_HEIGHT - 1,
      G2_LENS_WIDTH - 1,
      barTop + TOP_BAR_HEIGHT - 1,
      BORDER_VALUE,
    );
    ...
    if (state.closing) {
      ...
      const selectedTitle =
        state.windows[state.selectedIndex]?.title ?? "window";
    ...
    image.drawLine(
      ...
      barTop + 9,
      separatorX + 1,
      barTop + TOP_BAR_HEIGHT - 11,
      GLASS_TONE.track,
    );

    const maxIcons = notificationLayout.maxIcons;
    if (maxIcons > 0) {
      const { icons, stale } = readActiveNotificationIcons(
        maxIcons,
        renderPassAllowsStaleData(),
      );
      if (stale) {
      ...
      for (let index = 0; index < icons.length; index++) {
        const position = hudNotificationIconPosition(
          notificationLayout,
          index,
          {
            barTop,
  ...
  private drawPersistentHud(
    ...
    if (phone.battery !== null && Number.isFinite(phone.battery)) {
      items.push({
        kind: "phone",
        percent: phone.battery,
        charging: Boolean(phone.charging),
      });
    }
    if (
      state.battery.headset !== null &&
      Number.isFinite(state.battery.headset)
    ) {
      items.push({
        kind: "glasses",
        percent: state.battery.headset,
        charging: Boolean(state.battery.headsetCharging),
      });
    }
    if (state.ringConfigured) {
      const ringPercent =
        state.battery.ring !== null && Number.isFinite(state.battery.ring)
          ? state.battery.ring
          : null;
      items.push({
        kind: "ring",
        percent: ringPercent,
        charging: Boolean(state.battery.ringCharging),
      });
    }
    ...
    const badge = drawBrightnessBadge(
      font,
      brightness === "auto" ? "A" : brightness,
    );
    ...
    const bpmText =
      hr !== null && Number.isFinite(hr)
        ? String(Math.max(0, Math.min(255, Math.round(hr))))
        : "--";
    const bottomRow: HudItem[] = [
      { image: makeHeartRateHudItem(font, bpmText) },
    ];
    ...
    for (const kind of ["glasses", "ring", "phone"] as const) {
      ...
      if (item)
        bottomRow.push({
          image: makeBatteryHudItem(font, item, percentageMode),
        });
    }
    ...
    const bottomLeft = drawHudRow(
      image,
      bottomRow,
      HUD_CONTENT_RIGHT,
      barTop + HUD_ROW_HEIGHT,
      rowMinLeft,
    );
...
function makeBatteryHudItem(
  ...
  const valueWidth =
    percentageMode || item.percent === null
      ? font.measureText(percentText)
      : BATTERY_ICON_WIDTH;
  const labelIcon = batteryLabelIcon(item.kind);
  const labelGap = 5;
  const height = Math.max(font.lineHeight + 2, labelIcon.height, 16);
  const image = new GrayImage(
    labelIcon.width + labelGap + valueWidth,
    height,
    0,
  );
  image.bitBlt(
    labelIcon,
    0,
    Math.max(0, ((height - labelIcon.height) / 2) | 0),
    { transparentZero: true },
  );
  ...
  if (percentageMode || item.percent === null) {
    ...
    if (item.charging) {
      ...
      image.drawText(font, valueX, textY, percentText, GLASS_TONE.body);
    }
  } else {
    ...
    image.bitBlt(
      gauge,
      valueX,
      Math.max(0, ((height - gauge.height) / 2) | 0),
      { transparentZero: true },
    );
  }
  return image;
...
function makeHeartRateHudItem(
  ...
  const image = new GrayImage(
    HEART_ICON.width + gap + font.measureText(bpmText),
    height,
    0,
  );
  image.bitBlt(
    HEART_ICON,
    0,
    Math.max(0, ((height - HEART_ICON.height) / 2) | 0),
    { transparentZero: true },
  );
  image.drawText(
    font,
    HEART_ICON.width + gap,
    Math.max(0, ((height - font.lineHeight) / 2) | 0),
    bpmText,
    GLASS_TONE.body,
  );
  return image;
...
function bridgeHudLabel(bridge: ShellChromeState["bridge"]): string {
  ...
  switch (bridge.phase) {
    case "connected":
      return "HERMES GATEWAY ONLINE";
    case "connecting":
      return "HERMES GATEWAY CONNECTING";
    case "failed":
      return "HERMES GATEWAY ERROR";
    case "idle":
      return "HERMES GATEWAY OFFLINE";
  }
}

function makeBridgeHudItem(
  font: ReturnType<typeof getDefaultSmallFont>,
  bridge: ShellChromeState["bridge"],
): GrayImage {
  ...
  const stroke = !bridge.configured
    ? GLASS_TONE.border
  ...
  image.bitBlt(glyph, padX, Math.max(0, ((height - glyph.height) / 2) | 0), {
    transparentZero: true,
  });
  image.drawText(
    font,
    padX + glyph.width + gap,
    Math.max(0, ((height - font.lineHeight) / 2) | 0),
    label,
    GLASS_TONE.body,
  );
...
function drawHudRow(
  ...
  const fitted = fitHudRowItemWidths(
    items.map((item) => item.image.width),
    rightEdge,
    minLeft,
    HUD_ITEM_GAP,
  );
  ...
  for (const { index, x } of fitted) {
    ...
    const y =
      rowTop + Math.max(0, ((HUD_ROW_HEIGHT - item.image.height) / 2) | 0);
...
const TAB_EXTEND = 1;
const TAB_STROKE = GLASS_TONE.muted;
...
function drawSelectionTab(
  ...
  if (focused) {
    for (let yy = top; yy < bottom; yy++) {
      ...
      image.fillRect(x, yy, right - x + 1, 1, GLASS_TONE.focus);
    }
    return;
  }
  ...
  for (let yy = top; yy < bottom; yy++) {
    image.setPixel(
      left + tabLeftInset(yy, top, bottom, TAB_RADIUS),
      yy,
      TAB_STROKE,
    );
  }
  image.drawLine(
    left + tabLeftInset(top, top, bottom - 1, TAB_RADIUS),
    top,
    right,
    top,
    TAB_STROKE,
  ...
  image.drawLine(
    left + tabLeftInset(bottom - 1, top, bottom - 1, TAB_RADIUS),
    bottom - 1,
    right,
    bottom - 1,
    TAB_STROKE,
...
function tabLeftInset(
  ...
  return Math.max(
    0,
    Math.round(radius - Math.sqrt(Math.max(0, radius * radius - dy * dy))),
  );
}
...
function drawPixelClock(image: GrayImage, now: Date, barTop: number): number {
  ...
  const date =
    `${WEEKDAYS[now.getDay()]} ${now.getDate()} ${MONTHS[now.getMonth()]}`.toUpperCase();
