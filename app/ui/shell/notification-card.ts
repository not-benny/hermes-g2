...
export class NotificationCardLayer implements Layer {
  ...
  paint(ctx: LayerContext, _paintBelow: PaintBelow): GrayImage {
    ...
    image.drawText(small, PADDING, 35, truncateText(small, title, contentWidth), 245);
    const lines = body === title ? [] : wrapText(small, body, contentWidth).slice(0, 2);
    for (let index = 0; index < lines.length; index++) {
      image.drawText(small, PADDING, BODY_Y + index * BODY_LINE_HEIGHT, lines[index]!, 175);
    }
    const footer = `${GESTURE_CLICK} open   ${GESTURE_DOUBLE_CLICK} dismiss`;
    image.drawText(small, PADDING, CARD_HEIGHT - 17, footer, 115);
