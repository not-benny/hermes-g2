...
| Reuse existing Hermes Agent bridge for QR | Misleading/blocked | Existing bridge is a hostname-verified private WSS glasses-agent transport, not a WhatsApp protocol endpoint; its licensing gate remains open | Cannot be safely rolled back as a WhatsApp feature because no WhatsApp interface exists; extending it would enlarge scope and couple unrelated outages | No existing QR UI/API/session contract; incompatible without a new protocol |
...
- Resolve the existing bridge's licensing/provenance; preserve mandatory WSS and add replay, timeout, reconnect, Android certificate-negative, and stolen-token tests.
