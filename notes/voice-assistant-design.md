...
> **Historical design record.** The active external architecture is now
> MCP-only and is maintained in `docs/hermes-mcp-architecture.md`. The OpenClaw
> plugin, dynamic `glasses_list_tools` / `glasses_call` proxy, and custom chat,
> Cockpit, and Companion channels described below are retired and unavailable.
> Keep this file as design history, not installation or workflow guidance.
...
```
  ...
  │                                            │              │
  │  AssistantLayer (final result UI) <─terminal┤              │
  │                                            │              │
  │                                   AssistantBackend        │
  │                                   ┌────────┴─────────┐    │
  │                                   │ Direct  │ External│    │
...
3. **`AssistantSession`**: one conversation. Owns history/turn state and
   exposes `sendUtterance(text, ctx)` and `cancel()`. Backends may report
   deltas/tool progress internally, but the shell treats those as private and
   presents only the authoritative terminal result.

4. **`AssistantLayer`**: compact, viewport-aware final-result card. It never
   paints reasoning, drafts, or tool identities. Completed answers use direct
   gestures (scroll pages, click follow-up, double-click close).
...
**bridge server** co-located with the user's agent. One authenticated
websocket carries transport control plus two independent MCP sessions:

```
{ v: 1, chan: "ctl" | "mcp" | "host-mcp", ... }

ctl:  hello {deviceName, token, capabilities}, hello-ack, ping/pong, error
mcp:  phone-hosted private Device MCP      (host client -> phone server)
host-mcp: Hermes Host Session MCP          (phone client -> host server)
```

On `mcp` the phone is an MCP server. Its raw ToolRegistry remains private to
the authenticated native adapter; it is never dynamically mounted into the
model's tool inventory. A separately packaged workflow MCP exposes a static,
reviewed set of user-intent tools and invokes only pinned internal phone
routes.

On `host-mcp` Hermes is the MCP server. The phone submits voice through
`hermes.voice.turn`, receives one terminal result, reads bounded status
resources, and uses standard MCP cancellation. The custom chat, Cockpit, and
Companion channels are retired; a peer without Host MCP is rejected.

### Agent-side adapters

The native platform plugin owns authenticated WSS transport, exact active-turn
binding, the private Device MCP client, and Host MCP server. The portable
`hermes-g2-workflows` Agent Plugin owns the model-visible intent schemas and
workflow policy. A host-signed, package-digest- and turn-bound capability lets
that package reach a fixed native relay without exposing a generic phone call,
terminal, or dynamic tool-discovery surface.

### Proactive agent actions

Background delivery is not ordinary model authority. A deterministic native
producer may enqueue a previously authorized reminder or completed result
through one fixed proactive Device MCP route. Gating:

- Background producers have distinct code-owned capability and exact route
  allowlists; an interactive workflow or stale turn cannot inherit them.
- Master setting `assistant.allowProactive` (default on - it's a marquee
  feature - but visible and easy to turn off).
- Operation IDs, payloads, durable receipts, retry rules, and display gates
  are validated in code rather than prompts or `SOUL.md`.
...
`DirectAnthropicBackend` + `AssistantSession`; compact final-only `AssistantLayer`
