...
surface produced acknowledged compositor image updates. Android SAF then opened
a 12 KB synthetic Markdown fixture on the A32; synthetic G2 scroll changed the
reader-surface fingerprint, and auto-scroll advanced it at the configured
cadence before pause. Host lifecycle tests prove bookmark, restart metadata,
stale timer, foreground, screen, replacement, and close semantics. Direct worn
ring/touch bookmark and restart-restore still need a final interactive evidence
pass before this candidate can claim the complete hardware acceptance matrix.
