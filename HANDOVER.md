...
The focused PR #4 successor propagates opaque, generation-specific app-tool leases through the registry, in-process adapter, and worker windows. Stale or repeated teardown cannot remove a same-windowId replacement; replacing an open worker window releases its still-current lease before installing the new lifecycle state, including when the replacement closes before declaring tools. Focused registry/in-process tests pass 9/9. `npm run typecheck` fails on the repository's inherited 35 NativeScript Android/AndroidX/Java and `Array.create` diagnostics, with no changed-file diagnostic reported. No build or hardware verification was needed for this registry lifecycle change.
