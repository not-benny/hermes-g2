...
typedef void (*display_gate_fn)(void);               /* display semaphore give */
...
typedef uint32_t (*mutex_new_fn)(const void *attr);
typedef int32_t (*mutex_acquire_fn)(uint32_t mutex, uint32_t timeout);
typedef int32_t (*mutex_release_fn)(uint32_t mutex);
typedef int32_t (*mutex_delete_fn)(uint32_t mutex);
typedef uint32_t (*raw_sem_take_fn)(uint32_t sem, uint32_t timeout);
typedef uint8_t *(*container_create_fn)(uint32_t, uint32_t);
typedef void (*container_destroy_fn)(uint8_t *state);

/* firmware entry points (Thumb bit set for blx via constant pointer) */
#define FW_MALLOC  ((malloc_fn)0x00475a97U)         /* FUN_00474cd2 malloc(size) */
#define FW_FREE    ((free_fn)0x00475adbU)           /* FUN_00474d16 free(ptr) */
#define FW_INIT2   ((inflateInit2_fn)0x005c4e77U)   /* FUN_005beac2 inflateInit2_ */
#define FW_INFLATE ((inflate_fn)0x005c4f45U)        /* FUN_005beb90 inflate */
#define FW_END     ((inflateEnd_fn)0x005c4e3bU)     /* FUN_005bea86 inflateEnd */
#define FW_LOADBMP ((loadbmp_fn)0x004e106bU)        /* FUN_004dc5ae set_image_data / BMP decoder */
#define FW_FLUSH   ((cacheflush_fn)0x00475ed3U)     /* FUN_0047510e dcache clean range */
#define FW_SETSRC  ((lv_set_src_fn)0x00499df5U)     /* FUN_00498680 lv_image_set_src */
#define FW_INVAL   ((lv_invalidate_fn)0x00440657U)  /* FUN_00440656 lv_obj_invalidate */
#define FW_SIDE    ((lens_side_fn)0x0045a891U)       /* FUN_0045a568 -> 2=left, 1=right */
#define FW_BUZZ_PRESET ((buzz_preset_fn)0x0050767bU) /* FUN_00502b5a DRV_BuzzerPlayAfterQueue(type 0..8) */
#define FW_BUZZ_NOTE   ((buzz_note_fn)0x00507719U)   /* FUN_00502bf8 DRV_BuzzerPlayNote(note,tone,beat) */
#define FW_BUZZ_RESET  ((buzz_reset_fn)0x005075e5U)  /* FUN_00502ac4 buzzer stop/reset */
#define FW_BUZZ_RAW    ((buzz_raw_fn)0x005077a9U)     /* FUN_00502c88 reset+power+PWM(freq,duty) */
#define FW_TIMER_START ((timer_start_fn)0x004497c1U)  /* FUN_00449498 osTimerStart(handle,ms) */
#define FW_TIMER_NEW   ((timer_new_fn)0x004496d9U)    /* FUN_004493b0 osTimerNew(cb,type,arg,attr) */
#define FW_TIMER_STOP  ((timer_stop_fn)0x00449801U)   /* FUN_004494d8 osTimerStop(handle) */
#define FW_KEEPALIVE_RESET ((keepalive_reset_fn)0x004e5777U) /* FUN_004e0cba: EvenHub keepalive
                                                     * counter (@0x200751d8) = 0. This is the exact
...
#define FW_LOOKUP        ((lookup_fn)0x004e578bU)    /* FUN_004e0cce(id) -> spec node; state=*(node+0x10) */
#define FW_COMPLETE_EMIT ((complete_emit_fn)0x004dee3fU) /* FUN_004da382: stock image-complete emitter */
#define FW_DISPLAY_SIGNAL ((display_gate_fn)0x00473fefU)  /* FUN_0047386a: give display semaphore */
#define FW_DISPLAY_QUEUE  ((display_queue_fn)0x004748a3U) /* FUN_00474066: queue type-3 refresh */
#define FW_DISPLAY_COPY   ((display_copy_fn)0x0046d02dU)  /* FUN_0046ca14: stock packed-buffer copy */
#define FW_COMPASS_START  ((compass_control_fn)0x0054b279U) /* FUN_005455e4 StartIMUCompassFunc */
#define FW_COMPASS_STOP   ((compass_control_fn)0x0054b301U) /* FUN_0054566c StopIMUCompassFunc */
#define FW_DISPLAY_EVENT_FORWARD ((display_event_forward_fn)0x0045fc9dU) /* FUN_0045f8fc */
#define FW_COMPASS_NOTIFY ((compass_notify_fn)0x0058c7fdU) /* FUN_0058705c navigation_notify_compass_changed_cmd */
#define FW_DISPLAY_FB     (*(uint8_t * volatile *)0x200007b8U) /* stock copier's 640x480 destination */
#define FW_MUTEX_NEW      ((mutex_new_fn)0x00449a45U)
#define FW_MUTEX_ACQUIRE  ((mutex_acquire_fn)0x00449adfU)
#define FW_MUTEX_RELEASE  ((mutex_release_fn)0x00449b45U)
#define FW_MUTEX_DELETE   ((mutex_delete_fn)0x00449b97U)
#define FW_RAW_SEM_TAKE   ((raw_sem_take_fn)0x00441c45U)
#define FW_DISPLAY_SEM    (*(volatile uint32_t *)0x200752fcU)
#define FW_CONTAINER_CREATE  ((container_create_fn)0x004e099bU)
#define FW_CONTAINER_DESTROY ((container_destroy_fn)0x004e14f9U)
#define CFW_WAIT_FOREVER 0xffffffffU
#define BUZZ_TIMER_ADDR 0x20075130U                   /* RAM: buzzer osTimer handle (buzzer osTimer handle global) */
#define ZLIB_VER   ((const char *)0x0079710cU)      /* "1.1.4" */
 ...
 *
 * To convert cycles->us we need the core clock. The guessed global 0x20074e60 reads 0
 ...
 * (RAM 0x2007568c, SysTick chain) — that IS cycles-per-ms. Cached in the ctx; a bounded
...
#define FW_MS_TICK  (*(volatile uint32_t *)0x2007568cU)  /* firmware 1 ms OS tick (SysTick chain) */
#define FW_CORE_HZ  (*(volatile uint32_t *)0x20074e60U)  /* guessed core-clock global (reads 0 on hw) */
 ...
 * word of the BLE-RX task context (ble_msgrx, base = *0x004a1f74 ->
 * 0x200043b8; only its +0x8/+0xc are used by the firmware, and it is never freed).
...
typedef struct {
    ...
    cfw_owner owners[CFW_OWNER_MAX];
    uint32_t next_owner_generation;
    uint32_t snap_mutex;
    uint32_t worker_mutex;
    uint32_t timer_mutex;
...
#define CFW_CTX_SLOT  0x200043b8U    /* ble_msgrx context +0x0 (spare, never freed) */
#define CFW_CTX_MAGIC 0xC0FFEE64U    /* RTOS mutex + generation-qualified lifecycle ABI */

/* The snapshot producer and deferred consumer run on separate asynchronous
 * firmware paths. Protect their short metadata transactions by preserving and
 * disabling PRIMASK. Heap allocation, copying, frame work and frees stay
 * outside the critical section. Apollo510 is a single-core Cortex-M target. */
static uint32_t cfw_irq_save(void) {
    uint32_t irq_state;
    __asm volatile(
        "mrs %0, primask\n\t"
        "cpsid i\n\t"
...
static void cfw_irq_restore(uint32_t irq_state) {
    __asm volatile(
        "dsb\n\t"
        "msr primask, %0\n\t"
        "isb\n\t"
...
static int cfw_mutex_lock(uint32_t mutex) {
    return mutex != 0 && FW_MUTEX_ACQUIRE(mutex, CFW_WAIT_FOREVER) == 0;
}

static void cfw_mutex_unlock(uint32_t mutex) {
    if (mutex) (void)FW_MUTEX_RELEASE(mutex);
}

static int cfw_display_gate_take(uint32_t timeout) {
    uint32_t h = FW_DISPLAY_SEM;
    return h != 0 && FW_RAW_SEM_TAKE(h, timeout) == 1u;
}
...
void seq_tick(void *arg) {
    ...
    if (ctx->seq_count == 0) {
        ...
        return;                         /* stale callback must not silence a new preset */
    }
    if (ctx->seq_timer) (void)FW_TIMER_STOP(ctx->seq_timer);
    ...
    if (ctx->seq_timer == 0 || FW_TIMER_START(ctx->seq_timer, ms) != 0) {
        ctx->seq_count = 0;
        FW_BUZZ_RESET();
    }
    cfw_mutex_unlock(ctx->timer_mutex);
...
static int is_shadow_message(const uint8_t *src, uint32_t srclen) {
    ...
    /* Legacy BMP also writes the container's A buffer and retires any prior
     * direct-frame ownership, so it must quiesce the display task first. */
    return mode == 0x42 || mode == 3 || mode == 6 || mode == 8 || mode == 9;
}
...
static int image_worker(void *state_, uint8_t *src, uint32_t srclen) {
     ...
     * Stock firmware resets the ticks-since-last-heartbeat counter (@0x200751d8)
    ...
    if (gated) {
        if (ctx == 0) return -1;
        if (!cfw_display_gate_take(1000u))
            return -1;                              /* timeout: definitely not owner */
        owns_gate = 1;
        if (ctx->direct_pending) {
            FW_DISPLAY_SIGNAL();                    /* acquired, so release on anomaly */
            return -1;
        }
    }
    ...
    if (owns_gate && !rl.direct_submitted) FW_DISPLAY_SIGNAL();
...
static int image_dispatch(uint8_t *state, const uint8_t *src, uint32_t srclen,
    ...
    if (mode == 5) {
        /* Serialize sequence state and every related timer/buzzer side effect with
         * timer_mutex. A stopped callback may already be queued, so seq_count is
         * invalidated before any replacement sound is programmed. */
        uint8_t kind = (srclen >= 2) ? src[1] : 0xffu;
        customCfwContext *ctx = getCustomCfwContext();
        if (ctx == 0 || !cfw_mutex_lock(ctx->timer_mutex)) return -1;
        if (ctx->seq_timer) (void)FW_TIMER_STOP(ctx->seq_timer);
        ctx->seq_count = 0;
        ...
        if (kind == 0 && srclen >= 3) {                 /* preset 0..8 */
            ...
            if (duty > 100) duty = 100;
            if (ms < 1) ms = 1;
            FW_BUZZ_RAW(freq, duty);
            uint32_t h = *(volatile uint32_t *)BUZZ_TIMER_ADDR;
            if (h) (void)FW_TIMER_START(h, ms);
        } else if (kind == 4 && srclen >= 3) {           /* tone sequence */
            ...
            ctx->seq_cursor = 0;
            if (n && ctx->seq_timer == 0)
                ctx->seq_timer = FW_TIMER_NEW((void *)&seq_tick, 0, ctx, 0);
            if (n && ctx->seq_timer) {
                ctx->seq_count = (uint8_t)n;             /* publish sequence last */
                if (FW_TIMER_START(ctx->seq_timer, 1) != 0) {
                    ctx->seq_count = 0;
                    FW_BUZZ_RESET();
                }
            } else if (n) {
    ...
    if (mode != 3 && mode != 6) return -1;
    if (srclen < 3) return -1;

    const uint8_t *zsrc = src + 1;
    uint32_t zlen = srclen - 1;

    /* z_stream fields are accessed as pointers/u32 values below. Back the byte
     * view with uint32_t storage so those accesses are always 4-byte aligned. */
    uint32_t strm_words[ZS_SIZE / 4u];
    uint8_t *strm = (uint8_t *)(void *)strm_words;
...
static int load_bmp_fast(uint8_t *state, const uint8_t *bmp, uint32_t len) {
    ...
    if (ctx) {
        /* image_worker holds worker_mutex and the display semaphore here. Any
         * previously queued direct copy has completed before these fields and
         * the state-owned A buffer are changed. */
        ctx->direct_pending = 0;
        ctx->direct_shadow = 0;
        ctx->direct_active = 0;
        ctx->direct_owner = 0;
    ...
    uint32_t h = 0;
    if (!cfw_abs_bmp_height(bh_signed, &h))
        return FW_LOADBMP(state, (void *)bmp, len);
    ...
    if (w != *(uint16_t *)(state + 0x40) || h != *(uint16_t *)(state + 0x42) ||
        !cfw_bmp_pixel_region_fits(dataoff, stride, h, len))
        return FW_LOADBMP(state, (void *)bmp, len);
...
static customCfwContext *getCustomCfwContext(void) {
    ...
    customCfwContext *candidate =
        (customCfwContext *)FW_MALLOC(sizeof(customCfwContext));
    if (candidate == 0) return 0;
    bzero((uint8_t *)candidate, sizeof(customCfwContext));
    candidate->snap_mutex = FW_MUTEX_NEW(0);
    candidate->worker_mutex = FW_MUTEX_NEW(0);
    candidate->timer_mutex = FW_MUTEX_NEW(0);
    if (candidate->snap_mutex == 0 || candidate->worker_mutex == 0 ||
        candidate->timer_mutex == 0) {
        if (candidate->timer_mutex) (void)FW_MUTEX_DELETE(candidate->timer_mutex);
        if (candidate->worker_mutex) (void)FW_MUTEX_DELETE(candidate->worker_mutex);
        if (candidate->snap_mutex) (void)FW_MUTEX_DELETE(candidate->snap_mutex);
        FW_FREE(candidate);
        return 0;
    }
    candidate->diag_hide = 1;
    candidate->next_owner_generation = 1;
    for (uint32_t i = 0; i < CFW_FID_RING; i++) candidate->recent_fids[i] = 0xffff;
    candidate->magic = CFW_CTX_MAGIC;

    /* Only the final pointer publication is IRQ-masked. Allocation and all RTOS
     * operations happen before this tiny single-core compare-and-publish section. */
    uint32_t irq_state = cfw_irq_save();
    ctx = peekCustomCfwContext();
    if (ctx == 0) {
        *(customCfwContext **)CFW_CTX_SLOT = candidate;
        ctx = candidate;
        candidate = 0;
    }
    cfw_irq_restore(irq_state);

    if (candidate) {
        (void)FW_MUTEX_DELETE(candidate->timer_mutex);
        (void)FW_MUTEX_DELETE(candidate->worker_mutex);
        (void)FW_MUTEX_DELETE(candidate->snap_mutex);
        FW_FREE(candidate);
    }
    return ctx;
}
...
static void copy_panel(uint8_t *fb, const uint8_t *shadow) {
    /* Both addresses are firmware-owned byte pointers. Copy as bytes rather
     * than imposing an alignment/effective-type promise the ABI does not make. */
    for (uint32_t i = 0; i < PANEL_BYTES; i++) fb[i] = shadow[i];
}
...
/* ---- snapshot / restore and generation-qualified container lifecycle ---------- */

static int seq_before(uint32_t a, uint32_t b) {
    return (int32_t)(a - b) < 0;
}

static uint32_t cfw_next_generation_locked(customCfwContext *ctx) {
    uint32_t generation = ctx->next_owner_generation++;
    if (generation == 0) generation = ctx->next_owner_generation++;
    if (ctx->next_owner_generation == 0) ctx->next_owner_generation = 1;
    return generation;
}

static cfw_owner *cfw_owner_find_locked(
    customCfwContext *ctx, uint8_t *state, int require_alive
) {
    for (uint32_t i = 0; i < CFW_OWNER_MAX; i++) {
        cfw_owner *owner = &ctx->owners[i];
        if (owner->state == state && (!require_alive || owner->alive)) return owner;
    }
    return 0;
}

static cfw_owner *cfw_owner_register_locked(customCfwContext *ctx, uint8_t *state) {
    cfw_owner *owner = cfw_owner_find_locked(ctx, state, 1);
    if (owner) return owner;
    for (uint32_t i = 0; i < CFW_OWNER_MAX; i++)
        if (!ctx->owners[i].alive && ctx->owners[i].state == state) {
            owner = &ctx->owners[i];
            break;
        }
    if (owner == 0)
        for (uint32_t i = 0; i < CFW_OWNER_MAX; i++)
            if (!ctx->owners[i].alive && ctx->owners[i].state == 0) {
                owner = &ctx->owners[i];
                break;
            }
    if (owner == 0) {
        ctx->f_snap_of = 1;
        return 0;                         /* table exhausted: never alias a live owner */
    }
    owner->state = state;
    owner->generation = cfw_next_generation_locked(ctx);
    owner->alive = 1;
    return owner;
}

static uint32_t cfw_purge_owner_locked(
    customCfwContext *ctx, uint8_t *state, uint32_t generation,
    int match_generation, uint8_t **detached
) {
    uint32_t count = 0;
    for (uint32_t i = 0; i < CFW_SNAP_RING; i++) {
        cfw_snap *slot = &ctx->snaps[i];
        if (slot->state == state &&
            (!match_generation || slot->owner_generation == generation)) {
            if (slot->buf) detached[count++] = slot->buf;
            slot->state = 0;
            slot->buf = 0;
            slot->len = 0;
            slot->seq = 0;
...
static void cfw_free_detached(uint8_t **detached, uint32_t count) {
    for (uint32_t i = 0; i < count; i++) FW_FREE(detached[i]);
}

uint8_t *cfw_container_create(uint32_t arg0, uint32_t arg1) {
    uint8_t *state = FW_CONTAINER_CREATE(arg0, arg1);
    customCfwContext *ctx = getCustomCfwContext();
    if (state == 0 || ctx == 0 || !cfw_mutex_lock(ctx->snap_mutex)) return state;
    uint8_t *detached[CFW_SNAP_RING];
    uint32_t count = cfw_purge_owner_locked(ctx, state, 0, 0, detached);
    (void)cfw_owner_register_locked(ctx, state);
    cfw_mutex_unlock(ctx->snap_mutex);
    cfw_free_detached(detached, count);
    return state;
}

/* Quiesce workers before stock frees state/A/B. A pending direct-frame job must
 * also quiesce through the verified display semaphore; timeout leaks the stock
 * container deliberately rather than allowing a display-task use-after-free. */
void cfw_container_destroy(uint8_t *state) {
    if (state == 0) {
        FW_CONTAINER_DESTROY(state);
        return;
    }
    customCfwContext *ctx = getCustomCfwContext();
    if (ctx == 0) return;                            /* fail closed: leak without tombstone */

    uint8_t *detached[CFW_SNAP_RING];
    uint32_t count = 0;
    if (!cfw_mutex_lock(ctx->snap_mutex)) return;    /* fail closed: leak, never free live work */
    cfw_owner *owner = cfw_owner_find_locked(ctx, state, 0);
    if (owner && !owner->alive) {
        cfw_mutex_unlock(ctx->snap_mutex);
        return;                                      /* duplicate/in-progress destroy */
    }
    if (owner == 0) owner = cfw_owner_register_locked(ctx, state);
    if (owner == 0) {
        cfw_mutex_unlock(ctx->snap_mutex);
        return;                                      /* no durable tombstone: never free */
    }
    uint32_t generation = owner->generation;
    owner->alive = 0;                                /* tombstone blocks late re-registration */
    owner->generation = cfw_next_generation_locked(ctx);
    count = cfw_purge_owner_locked(ctx, state, generation, 1, detached);
    cfw_mutex_unlock(ctx->snap_mutex);
    cfw_free_detached(detached, count);

    if (!cfw_mutex_lock(ctx->worker_mutex)) return;
    if (ctx->direct_owner == state) {
        if (ctx->direct_pending) {
            if (!cfw_display_gate_take(1000u)) {
                cfw_mutex_unlock(ctx->worker_mutex);
                return;                             /* memory-safe fail-closed leak */
            }
            ctx->direct_pending = 0;
            ctx->direct_shadow = 0;
            ctx->direct_owner = 0;
...
int cfw_snapshot(uint8_t *state, uint32_t container_id) {
    (void)container_id;
    customCfwContext *ctx = getCustomCfwContext();
    if (ctx == 0 || state == 0 || !cfw_mutex_lock(ctx->snap_mutex))
        return (int)FW_SIDE();

    cfw_owner *owner = cfw_owner_find_locked(ctx, state, 0);
    if (owner && !owner->alive) {
        cfw_mutex_unlock(ctx->snap_mutex);
        return (int)FW_SIDE();                       /* teardown tombstone: never reactivate */
    }
    if (owner == 0) owner = cfw_owner_register_locked(ctx, state);
    uint8_t *b = *(uint8_t **)(state + 0xc);
    uint32_t len = *(uint32_t *)(state + 0x20);
    uint32_t capacity = *(uint32_t *)(state + 0x44);
    if (owner == 0 || b == 0 || len == 0 || len > capacity) {
        cfw_mutex_unlock(ctx->snap_mutex);
        return (int)FW_SIDE();
    }
    ...
    for (int i = 0; i < CFW_SNAP_RING; i++) {
        if (ctx->snaps[i].state == 0) {
            slot = i;
            break;
        }
        if (oldest_i < 0 || seq_before(ctx->snaps[i].seq, ctx->snaps[oldest_i].seq))
            oldest_i = i;
    }
    if (slot < 0) {
        slot = oldest_i;
        evicted = ctx->snaps[slot].buf;
        ctx->snaps[slot].state = 0;
        ctx->snaps[slot].buf = 0;
        ctx->snaps[slot].len = 0;
        ctx->snaps[slot].seq = 0;
        ctx->snaps[slot].owner_generation = 0;
        ctx->f_snap_of = 1;
    }
    ctx->snaps[slot].state = 0;
    ...
    ctx->snaps[slot].owner_generation = owner->generation;
    ctx->snaps[slot].state = state;                   /* validity marker last */
    cfw_mutex_unlock(ctx->snap_mutex);
    if (evicted) FW_FREE(evicted);
    return (int)FW_SIDE();
}
...
int image_deferred(uint8_t *state, uint8_t *src, uint32_t len) {
    customCfwContext *ctx = getCustomCfwContext();
    if (ctx == 0) return -1;                         /* never process state without worker lock */
    int r = 0;
    for (;;) {
        uint8_t *buf = 0;
        uint32_t blen = 0;
        ...
        int slot = -1;
        if (!cfw_mutex_lock(ctx->snap_mutex)) break;
        cfw_owner *owner = cfw_owner_find_locked(ctx, state, 1);
        if (owner) owner_generation = owner->generation;
        for (int i = 0; owner && i < CFW_SNAP_RING; i++)
            if (ctx->snaps[i].state == state &&
                ctx->snaps[i].owner_generation == owner_generation &&
                (slot < 0 || seq_before(ctx->snaps[i].seq, ctx->snaps[slot].seq)))
                slot = i;
        if (slot >= 0) {
            buf = ctx->snaps[slot].buf;
            blen = ctx->snaps[slot].len;
            ctx->snaps[slot].state = 0;
            ctx->snaps[slot].buf = 0;
            ctx->snaps[slot].len = 0;
            ctx->snaps[slot].seq = 0;
        ...
        if (!cfw_mutex_lock(ctx->worker_mutex)) {
            if (buf) FW_FREE(buf);
            break;
        }
        int valid = 0;
        if (cfw_mutex_lock(ctx->snap_mutex)) {
            owner = cfw_owner_find_locked(ctx, state, 1);
            valid = owner != 0 && owner->generation == owner_generation;
            cfw_mutex_unlock(ctx->snap_mutex);
        }
        if (valid) r = image_worker(state, buf, blen);
        cfw_mutex_unlock(ctx->worker_mutex);
        if (buf) FW_FREE(buf);
    }
    (void)src;
