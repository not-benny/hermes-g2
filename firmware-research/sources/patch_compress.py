...
DELTA = 0x37A1AD  # file_off = ghidra_addr - DELTA  (OTA mainApp component, 2.2.6.10)
...
LOADBMP_BL_SITE        = (0x498182, "48 f0 72 ff")
...
SNAPSHOT_BL_SITES      = {   # both decode to `bl 0x45a568` (verified)
    0x4e0424: "7a f7 34 fa",   # single-fragment complete
    0x4e0818: "7a f7 3a f8",   # multi-fragment last-fragment complete
}
# Command-ID 2 full local-settings response. The nearby 0x49d184 site is the
# distinct command-ID 1 acknowledgement path (the exact successor of upstream's
# intentionally-stock 0x49ba10), so it remains byte-for-byte stock.
SETTINGS_BL_SITE       = (0x49d2dc, "d9 f7 fc fa")  # bl FUN_00475b14 (aa21 send) -> wrapper
...
SETTINGS_DECODE_BL_SITE = (0x49c9dc, "f4 f7 5a ff") # bl FUN_00490120 -> settings_decode_wrapper
...
DISPLAY_START_BL_SITES = {
    0x45c9fa: "08 f0 88 fa",
    0x45caba: "08 f0 28 fa",
}
GESTURE_LONGPRESS_SITE = (0x442e92, "28 f0 0f fb")  # bl FUN_0046ae9c -> evenhub_longpress
GESTURE_RELEASE_SITE   = (0x4431c2, "1c f0 6b fd")  # bl FUN_0045f8fc -> ring_release
...
EVENAI_ENTRY_SITE      = (0x4e6a8e, "7f b5 06 00")
...
DISPLAY_COPY_BL_SITES = {
    0x474412: "f8 f7 0b fe",   # queue message type 3 -> bl (reloc 0x46d02c)
    0x47454e: "f8 f7 6d fd",   # queue message type 6 -> bl (reloc 0x46d02c)
}
...
WEAR_NOTIFY_BL_SITES = {
    0x4a03b0: "df f7 c2 fd",  # ON_HEAD:  bl 0x47ff38
    0x4a040e: "df f7 93 fd",  # OFF_HEAD: bl 0x47ff38
}
...
COMPASS_EVENT_BL_SITE = (0x443288, "1c f0 08 fd")  # bl FUN_0045f8fc(display,0x41,&heading)
# Generation-qualified ownership requires observing every stock container
# lifetime. These are the two direct constructor calls and sole direct destructor
# call in 8.4, all verified against stock disassembly and bytes.
CONTAINER_CREATE_BL_SITES = {
    0x496b7a: "49 f0 0e ff",
    0x4974fa: "49 f0 4e fa",
}
CONTAINER_DESTROY_BL_SITE = (0x495520, "4b f0 ea ff")
...
def layout(img):
    ...
    container_create_addr = base + _fn(built, "cfw_container_create")["offset"]
    container_destroy_addr = base + _fn(built, "cfw_container_destroy")["offset"]
    ...
    in_place = [
        ...
        (g2f(0x4e0a82), "bd f8 2c 10", "40 f2 41 20", "container width  <= 576"),
        (g2f(0x4e0b4a), "bd f8 2e 00", "40 f2 21 11", "container height movw #0x121"),
        (g2f(0x4e0b4e), "91 28",       "88 42",       "container height cmp r0,r1"),
        ...
        *[(g2f(site), orig, enc_bl(site, container_create_addr),
           f"bl cfw_container_create @ {site:#x} (register generation owner)")
          for site, orig in CONTAINER_CREATE_BL_SITES.items()],
        (g2f(CONTAINER_DESTROY_BL_SITE[0]), CONTAINER_DESTROY_BL_SITE[1],
         enc_bl(CONTAINER_DESTROY_BL_SITE[0], container_destroy_addr),
         "bl cfw_container_destroy (purge snapshots + quiesce workers)"),
    ]
